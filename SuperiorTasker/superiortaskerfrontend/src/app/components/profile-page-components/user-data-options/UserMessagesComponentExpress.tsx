...
import { connectSocketIo, disconnectSocketIo } from "@/app/server-actions/setupWebSocketSocketIoExpress";

export default function UserMessagesComponentExpres({ 
  user 
}: { 
  user: User 
}) {
  ...
  const handleNewMessage = (message: Message) => {
    if (currentPage === 0) {
      ...
      toast({
        title: 'New Message',
        description: `${message.firstName} ${message.lastName}: ${message.message}`,
        status: 'info',
        duration: 5000,
        isClosable: true,
      });
  ...
  useEffect(() => {
    if (user?.id) {
      connectSocketIo(user.id, handleNewMessage);
